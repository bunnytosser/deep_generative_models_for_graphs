...
class DGMG_new(nn.Module):
    ...
    def get_log_prob(self,shape1,shape2,shape3):
        ...
        """
        return torch.cat(self.add_node_agent.log_prob)[shape1:,:].sum()\
                + torch.cat(self.add_edge_agent.log_prob)[shape2:,:].sum()\
                + torch.cat(self.choose_dest_agent.log_prob)[shape3:,:].sum()
    def get_prob_dim(self):
        ...
        torch.tensor consisting of a float only
        """
        return torch.cat(self.add_node_agent.log_prob).size()[0]\
               ,torch.cat(self.add_edge_agent.log_prob).size()[0]\
               ,torch.cat(self.choose_dest_agent.log_prob).size()[0]


    def teacher_forcing(self, actions,scaf=True):
        ...
        while not stop_node:
            ...
            while not stop_edge:
                ...
                stop_edge, bond_type = self.add_edge_or_not(a=actions[self.step_count][1])
           # print("action:",actions[self.step_count][1])
            if scaf==True and self.step_count>=len(actions):
                stop_node=True
            else:
                stop_node = self.add_node_and_update(a=actions[self.step_count][1])
    ...
    def forward(self, scaffold,actions=None, rdkit_mol=False, compute_log_prob=False, max_num_steps=400):
        """
        Parameters
        ----------
        ...
        self.teacher_forcing(scaffold,scaf=True)
        shape1,shape2,shape3=self.get_prob_dim()
        self.step_count = 0

        if actions is not None:
            # A sequence of decisions is given, use teacher forcing
            self.teacher_forcing(actions,scaf=False)
        else:
            # Sample a molecule from the distribution learned by DGMG
            self.rollout(max_num_steps)

        if compute_log_prob and rdkit_mol:
            return self.get_log_prob(shape1,shape2,shape3), self.env.get_current_smiles()

        if compute_log_prob:
            return self.get_log_prob(shape1,shape2,shape3)
